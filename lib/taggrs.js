...
// Full decode: taggrs envelope + key → a plain request ready for the normal
// parser registry, or null. The decrypted `u` is a relative path (e.g.
// /g/collect?…&en=scroll…) rebuilt against the proxy host. The return also
// carries the opaque original (endpoint + the iv:ct blobs) so the panel can show
// the encrypted form alongside the decrypted one, mirroring the Stape b64 card.
export async function decodeTaggrsRequest(url, postData, keyString) {
  ...
  return {
    ...
    endpoint: url,                                        // where the opaque envelope was sent
    cipherU: env.u.iv + ':' + env.u.ct,                  // the encrypted original request
    cipherB: env.b ? env.b.iv + ':' + env.b.ct : null,   // the encrypted original body, if any
  };
}
