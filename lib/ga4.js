...
import { extractParams, hashAlgo, looksHashed, BUCKET_LABEL } from './params.js';
import { isSameSite } from './domain.js';   // eTLD+1 same-site test for the first-party call
...
export function parseGa4Request(url, postData) {
  ...
  if (isGoogleHost(host) && pathname.includes('/g/collect')) {
    ...
    if (decoded && decoded.kind === 'stape-b64') {
      ...
      // host). Only claim first-party when the endpoint is same-site as the page
      // (dl); without that evidence it's a proxy, not first-party.
      const u = new URL(url);
      const v = u.searchParams.get('v');
      const tid = u.searchParams.get('tid');
      if (v === '2' && tid && /^G-[A-Z0-9]+$/i.test(tid)) {
        let pageHost = '';
        try { pageHost = new URL(u.searchParams.get('dl') || '').host; } catch (e) { /* no dl */ }
        transport = pageHost && isSameSite(host, pageHost) ? 'first-party' : 'proxy';
      }
    }
  }
...
export function ga4PiiFields(userData, em) {
  ...
  (function walk(node) {
    ...
    for (const [k, v] of Object.entries(node)) {
      ...
      const bucket = udKeyToBucket(k);
      if (!bucket) continue;
      out[k] = { bucket, label: BUCKET_LABEL[bucket] || k, hashed: looksHashed(v), algo: hashAlgo(String(v)) };
    }
  })(userData);
  ...
  if (em) {
    ...
    for (const tok of s.split('~')) {
      ...
      out[key] = { bucket, label: BUCKET_LABEL[bucket] || key, hashed: looksHashed(val), algo: hashAlgo(val) };
    }
  }
