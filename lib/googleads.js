...
export function parseGoogleAdsRequest(url, postData) {
  ...
  //  - 'proxy'       : an Ads path served from some other domain (e.g. a shared
  //                    sGTM vendor host). Not the vendor, but not first-party
  //                    either; never claim first-party without the same-site match.
  const googleHost = isGoogleAdsHost(host);
  let transport;
  if (googleHost) {
    transport = 'google';
  } else {
    // The page origin travels inside the payload (dl/url); compare its eTLD+1.
    let pageHost = '';
    try { pageHost = new URL(get('url') || get('dl') || '').host; } catch (e) { /* no page url */ }
