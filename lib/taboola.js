...
import { extractParams, looksHashed, hashAlgo } from './params.js';
...
export function parseTaboolaRequest(url, postData) {
  ...
  const userData = unifiedId
    ? { unified_id: { bucket: 'email', label: 'Email (unified_id)', hashed: looksHashed(unifiedId), algo: hashAlgo(unifiedId) } }
    : null;
