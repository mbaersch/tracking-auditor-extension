...
// Taboola DOES support a hashed-email identity (AudienceMatch `unified_id`, the
// SHA-256 of the lower-cased email, set via _taboola.push({unified_id})), but it
// is opt-in and did not ride any captured hit: it must be set before the base tag
// initialises (like dataLayer before gtag) and/or needs the account provisioned,
// so a runtime push is dropped. We therefore surface no PII here — rather than
// guess a wire parameter we've never seen — and will wire it if a real capture
// ever shows how `unified_id` travels. Consent is surfaced verbatim (TCF `tcs`
// string + US-privacy `ccpaPs` + the CMP name `cbp`); the TCF string is not decoded.
...
export function parseTaboolaRequest(url, postData) {
  ...
  return {
    ...
    standardEvent,
    flags,
    userData: null,                    // no PII observed; the unified_id identity is opt-in (see header)
