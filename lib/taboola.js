...
// no hashed PII identifiers ride these requests, so identifiers stay empty.
// Consent is surfaced verbatim (TCF `tcs` string + US-privacy `ccpaPs` + the CMP
// name `cbp`); the TCF string is shown, not decoded.
...
export function parseTaboolaRequest(url, postData) {
  ...
  return {
    ...
    standardEvent,
    flags,
    userData: null,                    // no hashed PII on the Taboola pixel
