...
- Surfaces the user id (`ui`), page (`item-url`) / referrer and consent verbatim (the TCF
  ...
  Taboola tracks by cookie/id. Its hashed-email identity (AudienceMatch `unified_id`,
  SHA-256) is opt-in and did not appear in the captured hits — it must be set before
  the base tag initialises and/or needs account provisioning — so no PII is surfaced
  until a real capture shows how it travels. The `pre_d_eng_tb` engagement ping
  (time-on-site / scroll) is flagged as telemetry, not a real event.
