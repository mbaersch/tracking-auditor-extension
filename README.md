...
- Surfaces the user id (`ui`), page (`item-url`) / referrer and consent verbatim (the TCF
  ...
  Taboola tracks by cookie/id (no hashed PII on the pixel). The `pre_d_eng_tb` engagement
  ping (time-on-site / scroll) is flagged as telemetry, not a real event.
